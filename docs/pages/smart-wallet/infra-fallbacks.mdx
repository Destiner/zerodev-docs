...
ZeroDev offers a "[meta infrastructure](/meta-infra/intro)" that integrates with all leading AA infra providers.  When you use ZeroDev, you can pick [a specific infrastructure provider](/meta-infra/rpcs#configuring-the-rpc-for-a-specific-infra-provider).  If you don't pick one, we pick one for you.
...
const zeroDevPaymasterClient1 = createZeroDevPaymasterClient({
    chain,
...
const kernelClient1 = createKernelAccountClient({
    ...
    paymaster: {
        getPaymasterData(userOperation) {
            return zeroDevPaymasterClient1.sponsorUserOperation({userOperation})
        }
    },
})

const zeroDevPaymasterClient2 = createZeroDevPaymasterClient({
    chain,
...
const kernelClient2 = createKernelAccountClient({
    ...
    paymaster: {
        getPaymasterData(userOperation) {
            return zeroDevPaymasterClient2.sponsorUserOperation({userOperation})
        }
    },
})
```
